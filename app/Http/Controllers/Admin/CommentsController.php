...
class CommentsController extends Controller
{
    ...
    public function reopenticket(Request $req){
        ...
        try{
            ...
            if($reopenticket->tickettype == 'emalitoticket'){
                $replySubject = 'Re: ' . $reopenticket->subject;
                ...
                Mail::send([], [], function ($message) use ($reopenticket, $replySubject, $body) {
                    $message->to($reopenticket->cust->email)
                        ->from(setting('IMAP_USERNAME'))
                        ->subject($replySubject)
                        ->setBody($body, 'text/html');
                    $headers = $message->getHeaders();
                    $headers->removeAll('In-Reply-To');
                    $headers->removeAll('References');
                    $headers->addTextHeader('In-Reply-To', '<' . $reopenticket->MessageID . '>');
                    $headers->addTextHeader('References', '<' . $reopenticket->MessageID . '>');
                });
            }else{
