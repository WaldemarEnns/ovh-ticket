...
class AdminTicketViewController extends Controller
{
    ...
    public function selfassignticketview()
    {
        ...
        $selfassignedtickets = Ticket::where('selfassignuser_id', auth()->id())
            ->where('status', '!=' ,'Closed')
    ...
    public function myclosedtickets()
    {
        ...
        $myclosedbyuser = Ticket::select('tickets.*',"groups_categories.group_id","groups_users.users_id")
    ...
    public function mysuspendtickets()
    {
        ...
        $mysuspendtickets = Ticket::select('tickets.*',"groups_categories.group_id","groups_users.users_id")
            ->leftJoin('groups_categories','groups_categories.category_id','tickets.category_id')
            ->leftJoin('groups_users','groups_users.groups_id','groups_categories.group_id')
            ->whereIn('tickets.status', ['Suspend','On-Hold'])
            ->whereNotNull('groups_users.users_id')
            ->where('groups_users.users_id', Auth::id())
            ->latest('tickets.updated_at')
            ->groupBy('tickets.id')
    ...
    public function allactiveassignedtickets()
    {
        ...
        $allactiveassignedtickets = Ticket::whereIn('status', ['Re-Open','Inprogress','On-Hold'])->groupBy('tickets.id')->get();
