...
class AdminDashboardController extends Controller
{
    ...
    public function myassignedTickets()
    {
        ...
        $tickets = [];
        foreach($myassignedtickets as $key => $val){
            $ids = $myassignedtickets[$key]->ticketassignmutliples->pluck('toassignuser_id')->toArray();
            if(in_array(Auth::user()->id, $ids)){
                $tickets[$myassignedtickets[$key]->id] = $myassignedtickets[$key];
            }
        }

        $data['gtickets'] = $tickets;
