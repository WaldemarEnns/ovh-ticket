    ...
    public function show(Request $req, $id){
        ...
        if($req->ajax())
        {
            ...
            if($total_row > 0){
                ...
                foreach($data as $row){
                      // code by vikas---------
                    /*if((Auth::user()->id != $row->id) && (Auth::user()->getRoleNames()[0] =="Direction" || Auth::user()->getRoleNames()[0] =="BOSS" || Auth::user()->getRoleNames()[0] =="boss")){
                        if((!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '') == "IT"){
                            $output .= ' <option  value="'.$row->id.'"' .(in_array($row->id, $assugnuser_id)? 'selected': '').  '>'.$row->name.' ('.(!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '').')</option> ';
                        }
                    }
                    if((Auth::user()->id != $row->id) && (Auth::user()->getRoleNames()[0] =="RAFT" || Auth::user()->getRoleNames()[0] =="raft")){
                        if((!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '') == "Direction" || (!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '') == "BOSS" || (!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '') == "boss"){
                            $output .= ' <option  value="'.$row->id.'"' .(in_array($row->id, $assugnuser_id)? 'selected': '').  '>'.$row->name.' ('.(!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '').')</option> ';
                        }
                    }
                    if((Auth::user()->id != $row->id) && (Auth::user()->getRoleNames()[0] =="Modérateur" || Auth::user()->getRoleNames()[0] =="MODERATOR" || Auth::user()->getRoleNames()[0] =="moderator")){
                        if((!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '') == "IT" || (!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '') == "RAFT" || (!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '') == "raft"){
                            $output .= ' <option  value="'.$row->id.'"' .(in_array($row->id, $assugnuser_id)? 'selected': '').  '>'.$row->name.' ('.(!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '').')</option> ';
                        }
                    }
                    if((Auth::user()->id != $row->id) && Auth::user()->getRoleNames()[0] =="superadmin"){
                            $output .= ' <option  value="'.$row->id.'"' .(in_array($row->id, $assugnuser_id)? 'selected': '').  '>'.$row->name.' ('.(!empty($row->getRoleNames()[0])? $row->getRoleNames()[0] : '').')</option> ';
                    }*/
                    // original code ------
