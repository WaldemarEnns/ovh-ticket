												...
												<tbody>
													...
													<tr>
														...
														<td>
															...
															<span class="badge badge-burnt-orange">{{ $ticket->status }}</span>
										...
										<div class="card-header text-justified flex-1 pt-0 ps-md-3 ps-0 pb-0 ">
											<p class="fs-18 font-weight-semibold mb-1">{{setting('businesshourstitle')}}
												...
												@if(now()->timezone(setting('default_timezone'))->format('D') == $bussiness->weeks)
													...
													@if(strtotime($bussiness->starttime) <= strtotime(now()->timezone(setting('default_timezone'))->format('h:i A')) && strtotime($bussiness->endtime) >= strtotime(now()->timezone(setting('default_timezone'))->format('h:i A')))

														@if($bussiness->starttime != "24H")
															<span class="ms-3 badge bg-success text-white  mt-1 fs-12 font-weight-normal">online</span>
														@else
															<span class="ms-3 badge bg-success text-white  mt-1 fs-12 font-weight-normal">online</span>
														@endif
													@else
														@if($bussiness->starttime != "24H")
															...
															<span class="ms-3 badge bg-danger text-white  mt-1 fs-12 font-weight-normal">offline</span>
														@else
															<span class="ms-3 badge bg-danger text-white  mt-1 fs-12 font-weight-normal">offline</span>
														@endif
													@endif
						...
						<div class="col-xxl-9">
							<div class="card">
								<div class="card-header border-0 mb-1 d-block">
									...
									<small class="fs-13 "><i class="feather feather-clock text-muted me-1"></i>{{lang('Created Date')}} <span class="text-muted">{{$ticket->created_at->diffForHumans()}}</span></small>
								</div>
									...
									@if ($ticket->status != 'Suspend')
                                        @if($createdcount != null && $createdcount >= setting('MAXIMUM_ALLOW_REPLIES'))
                                            <div class="alert alert-danger mb-5 br-13" role="alert" id="reply_restrict">
                                                ...
                                                {{lang('You have reached maximum allowed replies to this ticket, please try')}} {{lang($difference)}}.
                                            </div>
                                        @else
