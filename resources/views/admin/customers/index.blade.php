							...
							<tr>
								...
								<td>
									...
									@if($customer->verified != 1 && $customer->userType == 'Customer')
										<a href="javascript:void(0)" data-id="{{$customer->email}}" id="resendverification" class="action-btns1" data-bs-toggle="tooltip" data-bs-placement="top" title="{{lang('Validate Customer')}}">
...
<script type="text/javascript">
	...
	(function($)  {
		...
		$('body').on('click', '#resendverification', function () {
			...
			swal({
				title: `{{lang('Are you sure you want to continue?', 'alerts')}}`,
				text: "{{lang('This action will Verify Customer to perform user level Action and Login', 'alerts')}}",
				icon: "warning",
				buttons: true,
				dangerMode: true,
			})
